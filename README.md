...
**一方通行の向きは頂点順しかない。** 共通規制種別コード11（一方通行）のレコードでは「進入方向」「禁止する方向」「指定する方向」がいずれも空で、方向を示すのは座標列の順序だけでした。**向きを逆に読むと逆走するネットワークができ、結果は一見もっともらしいのに全部間違います。** 利用側で OSM の `oneway` タグなどと突き合わせて検証してください。
